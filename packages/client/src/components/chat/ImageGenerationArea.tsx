...
const sampleTexts = [
  ...
  'A photorealistic portrait of a dog',
  'A minimalist logo for a startup',
  'An oil painting of a cat',
];

export default function ImageGenerationArea() {
  const [input, setInput] = useState('');

  const handleSubmit = () => {
    if (!input.trim()) return;
    console.log('Generate Image:', input);
    setInput('');
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 z-10 w-full max-w-4xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          <div className="w-16 h-16 rounded-2xl bg-blue-600 flex items-center justify-center shadow-[0_0_30px_rgba(37,99,235,0.4)] mb-6">
            <LucideImages className="w-8 h-8 text-white" />
          </div>
